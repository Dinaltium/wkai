...
# Pure ASCII on purpose (PowerShell mangles non-ASCII in some consoles).
...
if ($whip) { Write-Host "YES: $($whip.Line.Trim())" -ForegroundColor Green }
else { Write-Host "NO - this ffmpeg cannot do -f whip. Get FFmpeg 7.1+ full build." -ForegroundColor Red }

Section "Hardware H.264/HEVC encoders available"
...
if ($gdi) { Write-Host "YES: $($gdi.Line.Trim())" -ForegroundColor Green }
else { Write-Host "gdigrab not listed under -devices (still usually usable via -f gdigrab)." -ForegroundColor Yellow }

Section "Audio capture devices (DirectShow)"
$audio = ffmpeg -hide_banner -list_devices true -f dshow -i dummy 2>&1 |
  Select-String -Pattern "audio|Alternative name"
if ($audio) { $audio | ForEach-Object { $_.Line.Trim() } }
else { Write-Host "No DirectShow audio devices listed." -ForegroundColor Yellow }
